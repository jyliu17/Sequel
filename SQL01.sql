...
SELECT *
FROM employees
WHERE SALARY BETWEEN (SELECT MIN(salary) FROM employee) AND 2500;
...
SELECT first_name, last_name, department, city, state_province
FROM employees
...
SELECT employee_number, first_name, last_name, job_title
FROM employees
WHERE salary < (SELECT MIN(salary) FROM employees WHERE job_title = 'MK_MAN');
...
SELECT d.department_name, d.city, d.state_province
FROM departments d;
...
FROM employees e
WHERE e.department_id = (SELECT department_id FROM employees WHERE last_name = 'Taylor');
...
FROM employees
WHERE department_id IN (SELECT department_id FROM departments WHERE location = 'Toronto');
...
SELECT e.first_name, e.last_name, d.department_name, d.city
FROM employees e
JOIN departments d ON e.department_id = d.department_id;
...
WHERE l1.location_id = (SELECT l2.location_id FROM departments d2 JOIN locations l2 ON d2.location_id = l2.location_id WHERE d2.department_id = 30);
